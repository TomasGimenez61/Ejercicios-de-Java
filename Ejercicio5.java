		...
		while (contMenu == true) {
			...
			if (opMenu == 1) {
				while (seguir == 1 && i <= 20) {
					...
					if (opcion == 1) {
						...
						Administrativo nuevoEmp = new Administrativo(dniIng, nombreIng, apellidoIng, emailIng, opcion, hsExtraIng, hsMesIng, tipoEmpleadoIng);
						nuevoEmp.getsueldoFinal(SueldoBaseIng, hsExtraIng, hsMesIng);
						empleados[i] = nuevoEmp;
						i++;
					...
					if (opcion == 2) {
						...
						Vendedor nuevoEmp = new Vendedor(dniIng, nombreIng, apellidoIng, emailIng, SueldoBaseIng, porcenComisionIng, totalVentasIng, tipoEmpleadoIng);
			...
			if (opMenu == 2) {
				...
				for (int j = 0; j < i; j++) {
					...
					if (empleados[j].getTipoEmpleado() == 1) {
					...
					System.out.println();
					System.out.println("DNI del empleado: " + empleados[j].getDni());
					System.out.println();
					
					System.out.println("Nombre del empleado: "+ empleados[j].getNombre());
					System.out.println();
					
					System.out.println("Apellido del empleado: "+ empleados[j].getApellido());
					System.out.println();
					 
					System.out.println("Sueldo del empleado: " + empleados[j].getSueldoFinal());
					System.out.println();
				}
			}
